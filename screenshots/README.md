...
## Warehouse Access Control Validation

A dedicated extended ACL named `WAREHOUSE-RESTRICTIONS` applies least-privilege access control to traffic originating from the Warehouse network.
...
The tests were performed from `WAREHOUSE-PC (192.168.70.10)`.

![Warehouse Access Control Validation](warehouse-access-control-validation.png)

---

## Warehouse Router SSH — Authorized Management

SSH Version 2 is enabled on `WH-R1`.

A management ACL named `SSH-MANAGEMENT` restricts remote router administration to the IT/Security network.

Test:

`SECURITY-PC (192.168.20.10) → SSH → WH-R1`

Result: **Allowed**

![Warehouse SSH Authorized](warehouse-ssh-security-success.png)

---

## Warehouse Router SSH — Unauthorized Management Denied

A device within the Warehouse network is not permitted to remotely administer `WH-R1`.

Test:

`WAREHOUSE-PC (192.168.70.10) → SSH → WH-R1`

Result: **Denied**

![Warehouse SSH Unauthorized Denied](warehouse-ssh-unauthorized-denied.png)

---

## Centralized Syslog Monitoring

`INTERNAL-SERVER (192.168.50.10)` is configured as a centralized Syslog server for network-device event monitoring.

Both `GOV-R1` and `WH-R1` forward informational Syslog messages to the server using UDP port 514.

The captured logs demonstrate events generated by both routers, including configuration and interface state changes.

Result: **Centralized logging successfully validated**

![Centralized Syslog Monitoring](centralized-syslog-monitoring.png)
...
- Warehouse least-privilege access control
- Restricted SSH management of the Warehouse router
- Unauthorized Warehouse management access prevention
- Centralized Syslog monitoring
